...
#################################################
#camera
dit = 40               #  What integration time (millisec), will be set by server to closest possible integration time of the camera, which depends on setup.
coadds = 1             #  this should probably be 1
nseqs = 150            #  How many frames per 'go'?
ngos = 6               #  How many go's per nod?
...
N_cycles = 3           #  How many nodding cycles should be executed by this script with a single execution?
...
for ii in xrange(N_cycles):                                                        #  Execute all nodding cycles.
    ...
    print('Nodded.  Pausing for manual interaction.  Hit return to continue'); raw_input()
    for jj in range(0, ngos): 
