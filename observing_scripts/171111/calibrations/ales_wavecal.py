...
pi.setINDI("LMIRCAM.Command.text", "5 1 15 lbtintpar" , timeout=300)
for filt in ('NB3375-025','NB3555-041','NB3950-035'):
    print filt
    pi.setINDI("Lmir.lmir_FW2.command", filt, timeout=45, wait=True)
    pi.setINDI("LMIRCAM.Command.text", "go", timeout=600, wait=True)
    pi.setINDI("LMIRCAM.Command.text", "go", timeout=600, wait=True)

pi.setINDI("LMIRCAM.Command.text", "10 1 15 lbtintpar" , timeout=300)
for filt in ('NB2925-055',):
    print filt
    pi.setINDI("Lmir.lmir_FW2.command", filt, timeout=45, wait=True)
    pi.setINDI("LMIRCAM.Command.text", "go", timeout=600, wait=True)
    pi.setINDI("LMIRCAM.Command.text", "go", timeout=600, wait=True)
