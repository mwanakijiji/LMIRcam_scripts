...
#set filters 
pi.setINDI("Lmir.lmir_FW2.command", 'NB3555-041', timeout=45, wait=True)
pi.setINDI("Lmir.lmir_FW25.command", "Open", timeout=45, wait=True)
pi.setINDI("Lmir.lmir_FW3.command", "Open", timeout=45, wait=True)
pi.setINDI("Lmir.lmir_FW4.command", "Open", timeout=45, wait=True)
#turn on slow mode (also sets full frame)
...
#    dit = j * 1475.3 - 10.0     # Request DITs in steps of multiples of minimum DIT, but subtract a little so the camera rounds up to the correct DIT.  
