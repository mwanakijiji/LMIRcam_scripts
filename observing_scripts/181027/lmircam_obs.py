...
#################################################
#camera
dit = 500              #  What integration time (millisec), will be set by server to closest possible integration time of the camera, which depends on setup.
coadds = 2             #  this should probably be 1
nseqs = 75             #  How many frames per nod?
use_bg = 1             #  1 True, 0 False.  Use a previously taken background.
savedata = True       #  Wanna save data?

#telesecope
reverse = True        #  If True, this inverts the nodding cycle (e.g., it will start on the right instead of the left).
nod_x = 10              #  Nod throw in arcseconds in detector coords. If positive and inverse = False, the first nod will go to the right.
nod_y = 0              #  Nod throw in arcseconds in detector coords. If positive and inverse = False, the first nod will go up.
side = 'right'          #  Which side to nod.  Can be 'left', 'right', or 'both'.

N_cycles = 20          #  How many nodding cycles should be executed by this script with a single execution?
