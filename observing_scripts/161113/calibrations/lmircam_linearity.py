...
#save frames
#for j in xrange(30):
for j in np.r_[0,np.logspace(0,np.log10(17),30)]:
    print j
    pi.setINDI("LMIRCAM.Command.text", "%f 1 %i lbtintpar" % (j,N_IMS), timeout=300)
    time.sleep(0.5)
...
#for j in xrange(30):
for j in np.r_[np.logspace(0,np.log10(17),30)]:
    print j
    pi.setINDI("LMIRCAM.Command.text", "%f 1 %i lbtintpar" % (j,N_IMS), timeout=300)
    time.sleep(0.5)
    pi.setINDI("LMIRCAM.Command.text", "go", timeout=300)
