...
###put FW2 back
    pi.setINDI("LMIRCAM.Command.text", "0 contacq %f %i %i lbtintpar" % lightIntParams, wait=True)
    pi.setINDI("Lmir.lmir_FW2.command", int(fw2_pos), timeout=20, wait=True)
###put the integration parameters back
    
pi.setINDI("LMIRCAM.Command.text", "0 loglevel 0 contacq 1 savedata %f %i %i lbtintpar" % lightIntParams, wait=True)
...
# take frames and save the data
    print 'DO PRIMARY'
    print 
    pi.setINDI("LMIRCAM.EditFITS.Keyword=FLAG;Value=PRI;Comment=observation type", wait=False)
    time.sleep(1)
    pi.setINDI("LMIRCAM.Command.text","go",timeout=300,wait=True)
