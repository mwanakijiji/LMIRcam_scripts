...
def nod_sneakSomeDarks(RAOff,DecOff,darkIntParams,side):
    ...
    pi.setINDI("LMIRCAM.Command.text","go",timeout=4*np.product(darkIntParams),wait=True)
...
def do_exposures(Lintparams,Nintparams,flag, side):
    ...
    pi.setINDI("NOMIC.Command.text","go",timeout=4*np.product(Nintparams),wait=False)
    pi.setINDI("LMIRCAM.Command.text","go",timeout=4*np.product(Lintparams),wait=True)
