...
#################################################
#camera
dit = 300               #  What integration time (millisec), will be set by server to closest possible integration time of the camera, which depends on setup.
...
nod_y = 2.3             #  Nod throw in arcseconds in detector coords. If positive and inverse = False, the first nod will go up.
side = 'right'          #  Which side to nod.  Can be 'left', 'right', or 'both'.

N_cycles = 2            #  How many nodding cycles should be executed by this script with a single execution?
...
for ii in xrange(N_cycles):                                                        #  Execute all nodding cycles.
    ...
    pi.setINDI('NOMIC.Command.text', 'go', timeout=50000, wait=False)
