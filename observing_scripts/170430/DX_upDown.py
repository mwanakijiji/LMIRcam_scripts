...
#############################
NUMBER_OF_NOD_PAIRS = 1
NOD_DISTANCE_ASEC = 5.0
...
for j in range(NUMBER_OF_NOD_PAIRS):
    ...
    pi.setINDI("LMIRCAM.Command.text","rawbg",timeout=600,wait=False)
...
#set the nod up vector
    ...
    wait4AORunning() 
    #time.sleep(1)
pi.setINDI("LMIRCAM.Command.text","0 savedata",timeout=600,wait=True)
