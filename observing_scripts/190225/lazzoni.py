...
#################################################
#camera
dit = 1200             #  What integration time (millisec), will be set by server to closest possible integration time of the camera, which depends on setup.
dit_short = 200
...
nod_y = 8              #  Nod throw in arcseconds in detector coords. If positive and inverse = False, the first nod will go up.
...
for ii in xrange(N_cycles):                                                        #  Execute all nodding cycles.
    ...
    get_lmircam_frames(dit_short, coadds, nseqs_short, use_bg=use_bg, save_data=savedata)      #  
