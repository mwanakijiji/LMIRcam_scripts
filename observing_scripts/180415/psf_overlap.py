...
def findStar(image, cx, cy, PSFside):
    if autoFindStar:
	...
	'''
        ...
        imageG = ndimage.gaussian_filter(imageThis, 6) # remove effect of bad pixels (?)
...
for k in range(3): # do three iterations to get back to the original position
	...
	PSFside = 'left'
	[current_cy_L,current_cx_L] = findStar(imgSubBP, cx, cy, 'left')
	print 'LEFT:'
	print [current_cy_L,current_cx_L]

	PSFside = 'right'
	print 'RIGHT:'
	[current_cy_R,current_cx_R] = findStar(imgSubBP, cx, cy, 'right')
	print [current_cy_R,current_cx_R]
    #pdb.set_trace()
