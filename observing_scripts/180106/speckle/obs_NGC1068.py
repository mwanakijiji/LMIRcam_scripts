...
# camera
dit = 0         # Minimum DIT, may need to be changed depending on SNR (reasonable SRN without saturating, DIT 
...
for i_nod in range(i_nods_done, 6):
  ...
  if i_nod != i_nod_start or skip_first_int == False:
    for i_ao in range(0, 3):
      ...
      pi.setINDI('LMIRCAM.Command.text','go', timeout=10000, wait=True)
