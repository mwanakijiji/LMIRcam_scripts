...
if firstPositions: # if this is the original nod, this loop will find PSF positions
	...
	#### PROCESS ORIGINAL IMAGE, UP NOD
	imgSubBP = processImg(img,'median')
	#print img
	#print imgSubBP

	PSFside = 'left'
	[orig_cy_L_up,orig_cx_L_up] = findStar(imgSubBP, cx, cy)
	print [orig_cy_L_up,orig_cx_L_up]

	PSFside = 'right'
	[orig_cy_R_up,orig_cx_R_up] = findStar(imgSubBP, cx, cy)
	print [orig_cy_R_up,orig_cx_R_up]

	pi.setINDI("LMIRCAM.Command.text","rawbg",timeout=300,wait=False)

	#### NOD DOWN AND PROCESS ORIGINAL IMAGE IN DOWN NOD
	pi.setINDI("LBTO.OffsetPointing.CoordSys", "DETXY","LBTO.OffsetPointing.OffsetX", 0.0, "LBTO.OffsetPointing.OffsetY", -4.5, "LBTO.OffsetPointing.Side", "both", "LBTO.OffsetPointing.Type", "REL", timeout=400) 
	...
	PSFside = 'left'
	[orig_cy_L_down,orig_cx_L_down] = findStar(imgSubBP, cx, cy)
	print [orig_cy_L_down,orig_cx_L_down]

	PSFside = 'right'
	[orig_cy_R_down,orig_cx_R_down] = findStar(imgSubBP, cx, cy)
	print [orig_cy_R_down,orig_cx_R_down]
...
sys.exit()
########################################################################################
## take more up-down pairs, correcting position of each PSF to make the original ones ##
########################################################################################
...
for j in range(10):
	...
	#################### start finding position in up nod

	# take 1 image to test per nod position
	pi.setINDI("LMIRCAM.Command.text","0 savedata") # don't save this
	pi.setINDI("LMIRCAM.Command.text","1 obssequences",timeout=300,wait=True)
	f=pi.getFITS("LMIRCAM.DisplayImage.File", "LMIRCAM.GetDisplayImage.Now", wait=True)
	f.verify('fix')
	head = f[0].header
	...
	PSFside = 'left'
	[new_cy_L_up,new_cx_L_up] = findStar(imgSubBP, cx, cy)
	#print 'LEFT:'
	#print [new_cy_L_up,new_cx_L_up]

	PSFside = 'right'
	[new_cy_R_up,new_cx_R_up] = findStar(imgSubBP, cx, cy)
	#print 'RIGHT:'
	#print [cy_R_up,cx_R_up]
	
	#################### end finding position in up nod


	#################### start making correction
