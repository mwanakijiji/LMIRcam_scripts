...
#ES Jan 6, 2015-- increased the steps in the four-loop to do four-point dithers for a single target, where the target needs to appear in the center of each quadrant
...
for j in range(10):
	...
	pi.setINDI("LBTO.OffsetPointing.CoordSys", "DETXY","LBTO.OffsetPointing.OffsetX", 5.0, "LBTO.OffsetPointing.OffsetY", 0.0, "LBTO.OffsetPointing.Side", "both", "LBTO.OffsetPointing.Type", "REL") 
#---
        wait4AORunning()
	...
	#Note that these are X and Y values of the star on the array where LL is 0,0.
	#There is a rotation and sign flip in Elwood's INDI-IIF interface to make this work.
	pi.setINDI("LBTO.OffsetPointing.CoordSys", "DETXY","LBTO.OffsetPointing.OffsetX", 0.0, "LBTO.OffsetPointing.OffsetY", -5.0, "LBTO.OffsetPointing.Side", "both", "LBTO.OffsetPointing.Type", "REL") 
#---
        wait4AORunning()
	pi.setINDI("LMIRCAM.Command.text","1000 sleep")
	pi.setINDI("LMIRCAM.Command.text","go",timeout=300,wait=True)
